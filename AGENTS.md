...
5. Read the minimum necessary files; make surgical changes only. A path, symbol, or flag the request names may not exist — check before editing, and never silently substitute the nearest similarly-named file. Don't start multi-file work near the context limit — summarize state and hand off instead.
