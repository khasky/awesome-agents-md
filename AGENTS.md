...
4. Work that writes code — a feature, a fix, a refactor, a migration — runs in a dedicated `git worktree` on its own branch, never in the shared main checkout, so a parallel session or the user's own edits can't collide with it; read-only work (investigation, review, audit, a question) stays where it is. Untracked local files and installed dependencies don't follow into a new worktree — copy or reinstall whatever the build needs there.
...
6. Verify per the gate above; report briefly. Feature finished and its commits pushed → offer to remove the worktree (`git worktree remove`) so the checkouts don't pile up; removing it is the user's call, never yours.
