...
Delegating to subagents: a spawned task is not a completed task — if you delegate, you own collecting and integrating the results before your final message; fire-and-forget is forbidden. Decompose only when the work can't fit one context, or when exploration would flood the main context with file dumps better isolated in a subagent. Subagents return conclusions, not raw dumps: give each an explicit output contract (a `path:line` list, a diff receipt) and a scope cap it must refuse beyond ("too big: split into N tasks") instead of half-doing. A subagent that verifies gets the diff and the criteria, never the implementer's reasoning — justification is what talks a reviewer into a pass. Parallel subagents get disjoint files, a timeout, and a cap on how many run at once; work that needs one agreed design is a single agent's job, not a fleet's. Every delegation names the model tier with the task, plus the reasoning effort where the harness exposes one, and the cheapest setting that can produce the required output wins: search, file location, inventories, mechanical edits, format conversion and reading a named file back all take the light tier, while trade-off decisions, a stubborn root cause and a review nobody will re-check are what earn the expensive one. Tiers are a capability class, not a name you remember — resolve the active harness's own name for the tier at use time, pin it in a reusable agent's definition file rather than re-deciding per call, and never let a child inherit the parent's tier by default (`rules/subagents.md`). Every spawned agent is told it shares the workspace — it must not revert, reformat or stage what another agent or the user has in flight — and is told outright whether it may spawn agents of its own; an unstated recursion limit is how one fleet becomes several. Collect or close every agent you start.
...
- `rules/long-running-agents.md` — unattended multi-iteration runs: state baton, early stops, stall detection, quota-vs-error branching, budget.
