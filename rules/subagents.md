...
- A reusable agent gets its tier in its definition file (a `model:` field in the agent's frontmatter, where the harness has one), so it is decided once; an ad-hoc spawn passes the tier in the spawn call.
