...
Read this when writing or reviewing client-side state: component state, global stores, persisted state, route state. The rules hold for any client framework; the JS/TS library APIs sit under their own heading.
...
- URL search params are user input: schema-validate them at the route boundary, never read raw strings deep in components (core trust-boundary rule).
...
- Persist by field allowlist, never the whole store; tokens, PII, and authorization decisions never go to web storage (`rules/backend-security.md`). Anything persisted beyond trivial preferences carries a schema version and a migration path — silent shape drift after a deploy breaks returning users.
...
- Distinguish "no data yet" from "refreshing" and keep previous data while paginating — a full-page spinner on every page change is a regression dressed as loading UX.

## JS/TS library specifics

- TanStack Router: `validateSearch` is the schema boundary for URL params.
- Zustand persist: `partialize` is the field allowlist; pair it with `version` and `migrate`.
- TanStack Query: `isLoading` is "no data yet", `isFetching` is "refreshing"; keep previous data across page changes with `placeholderData: keepPreviousData` (v5 — the old `keepPreviousData: true` option is gone).
