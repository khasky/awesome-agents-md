...
- Response caching is explicit, never accidental: `Cache-Control: no-store` for authenticated or personalized responses; cacheable GETs declare `public, max-age` for browsers plus `s-maxage` for the CDN, with `stale-while-revalidate` where brief staleness is acceptable. `Vary` names every request header that changes the response. Plan CDN invalidation (purge key or short TTL) when the endpoint is designed — a cached response you cannot purge is a bug with a timer.
...
- Rate-limit headers so clients self-throttle: `Retry-After` on 429 always, plus quota headers — the de-facto `X-RateLimit-Limit`/`-Remaining`/`-Reset` triple or the IETF draft's `RateLimit`/`RateLimit-Policy` pair (still a draft; its field names have changed across revisions — pick one scheme and document it). Key limits by API key, then IP.
