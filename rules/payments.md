...
- Money is integer minor units end to end — never a float, never a locale-formatted string. Round once, at a documented point, and take the currency's exponent from the provider instead of assuming two decimals (JPY has none, KWD has three).
...
- Never store raw card data; keep the provider's secret API key and webhook signing secret server-only and out of the client bundle and any tracked file (`rules/backend-security.md`).
