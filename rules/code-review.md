...
- The opening names what changed, never how many. "correct seven behaviors" and "fixes three modules" send the reader into the diff to learn which seven; naming them costs the same room and answers the question where it was asked, and the shape of the change is what the first sentence exists to give. Trim a long list with "and others" instead of replacing it with its length. A contrast frame in front of the opening goes the same way, deleted with nothing put in its place: "Four typecheck configurations run, not one:" and "One thing worth naming rather than hiding:" both stand in front of sentences that already make the point (`rules/code-comments.md`). This binds the title too, which a squash merge turns into the commit subject and the changelog line.
...
- A change that alters no behavior - a comment pass, a rename of prose, a docs-only edit - answers `Verification` and `Confidence` in a line each: there was nothing to run, and behavior is unchanged. Say what was checked instead ("each rewritten comment against the code it sits on") and stop. Reasoning that holds for every such change is padding a reviewer already knows from the diff: that a comment is not executed, that no lint reads a claim, that nothing outside the repository moves. `Problem` keeps only what specifically produced this drift - the mechanism that moved and left the prose behind - never the general observation that prose and code drift apart.
