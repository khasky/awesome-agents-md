...
- Run schema migrations as a separate init job or pre-deploy step, never in the serving container's start command or lazily inside request handling — a `migrate && start` entrypoint runs the migration once per replica and couples schema state to instance count mid-rollout (`rules/deployment.md`).
- One process concern per container; logs go to stdout for the runtime to collect (`rules/observability.md`); keep the container stateless — uploads/sessions/cache go to external stores (same rule as `rules/backend-security.md` horizontal-scaling statelessness, repeated here because it decides what gets a volume).
