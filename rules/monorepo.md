...
- Dependencies point one way: applications depend on packages, never the reverse. A package meant for more than one environment (browser, server, mobile, CLI) takes an environment-specific capability through an interface the consuming application implements, instead of importing it (`rules/design-patterns.md`).
