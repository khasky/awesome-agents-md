...
- Verify provenance where the registry supports it (`npm audit signatures`, PyPI attestations, sigstore); prefer packages that publish from a traceable build.
...
- Scan the built artifact (a package, an image, a binary) in CI and fail on fixable critical and high findings.
