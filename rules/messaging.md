...
- A subscriber gets its own dedicated connection: in brokers where a subscribed connection can't issue normal commands (Redis pub/sub — duplicate the client), sharing one connection deadlocks the app. Guard the message parse and drop malformed messages instead of crashing the consume loop.
