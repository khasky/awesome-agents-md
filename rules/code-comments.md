...
- No markdown backticks around identifiers: a developer typing a comment does not wrap names in backticks, and it is the loudest machine-written tell. Where dropping them leaves a bare token colliding with the prose, use straight double quotes (the "Log in" text). The explanation attached to a functional directive is part of that directive and stays verbatim, backticks included.
