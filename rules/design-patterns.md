...
- Never introduce a dependency cycle between modules: two modules that import each other are one module in two files. Break it by extracting the shared piece or inverting one edge. A cycle that was already there is reported as a finding, not fixed as a side effect of the task (core Coding rule).
